...
static esp_err_t teste_get_handler(httpd_req_t *req)
{
...
static esp_err_t putValues_post_handler(httpd_req_t *req)
{
    ...
    int ph = randomValueGenerator();
    int temperature = randomValueGenerator();
    int turbidity = randomValueGenerator();

    sprintf(json_response, "{\"ph\":%d, \"temperature\":%d, \"turbidity\":%d}", ph, temperature, turbidity);
...
static const httpd_uri_t hello = {
    ...
    .user_ctx  = "BABABABA        fiiiiiuu!\n\n\n\n\nmulher fdp"
};

static const httpd_uri_t teste = {   
    .uri       = "/teste",
    .method    = HTTP_GET,
    .handler   = teste_get_handler,
    /* Let's pass response string in user
     * context to demonstrate it's usage */
};
...
esp_err_t http_404_error_handler(httpd_req_t *req, httpd_err_code_t err)
{
    if (strcmp("/hello", req->uri) == 0) {
        ...
        return ESP_OK;
    } else if (strcmp("/bunda", req->uri) == 0) {
...
static esp_err_t ctrl_put_handler(httpd_req_t *req)
{
    ...
    if (buf == '0') {
        ...
        httpd_register_uri_handler(req->handle, &teste);
        httpd_register_uri_handler(req->handle, &put_data);
